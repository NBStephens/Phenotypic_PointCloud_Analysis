...
def get_mean_point_distance(point_cloud_model):
    ...
    distance_df = pd.DataFrame(distance.cdist(point_cloud_model, point_cloud_model, metric='euclidean'))
...
def expected_ec_3d(resels, pvalue, degrees_free):
    ...
    EC = resels * ((((4*np.log(2))**(3/2))/((2*np.pi)**2)) * (np.exp(((-1 * z)**2)/2)*(z**2-1)))
    pvalue_thresh = np.where(EC < pvalue)[0]
    ...
    EC_3d = resels * (((4*np.log(2))**(3/2))/((2*np.pi)**2)) * ((1 + ((z**2) / degrees )) ** ((-1/2) * (degrees - 1))) * (((degrees - 1)/degrees) * ((z ** 2) - 1))
...
def linear_comparison(outname, frame, canonical_geo, uvalue):
    ...
    results_df['thresh'] = np.where(results_df['p-value'] > float(uvalue), 1.00, results_df['p-value'])
    results_df.to_csv("Linear_" + outname + ".csv", index=False, float_format='%.10f')
    scalars = results_df['thresh']
    write_case(outname, scalars=scalars, scalar_type="pvalues", canonical_geo=canonical_geo)
    end = timer()
    end_time = (end - start)
...
def ttest_comparison(outname, frame, canonical_geo, resels, pvalue=0.05):
    ...
    for p in possibilities:
        ...
        try:
            ...
            isolated_df['thresh'] = np.where(isolated_df['Absolute_T-score'] < float(uvalue),
            ...
            print(f"Sample size is too small for {p[0]} vs {p[1]} to obtain a correction value!")
        finally:
            end = timer()
            end_time = (end - start)
...
def get_necessary_files(matching_text="canonical"):
    ...
    try:
        ...
        print("Did not find canonical point cloud! Please make sure it is in the folder!")
    finally:
...
def get_data_files_for_scalars(group_list, group_identifiers, point_cloud_dir, scalar_list, max_normalixed=True):
    for scale in scalar_list:
        ...
        if scalar_dir.exists():
            ...
            data_files = glob.glob(str(pathlib.Path(scalar_dir.joinpath(f"*{scale}*original_mapped_{scale}.csv"))))
            if max_normalixed == True:
                max_data_files = glob.glob(str(pathlib.Path(scalar_dir.joinpath(f"*{scale}*original_mapped_max_normalized_{scale}.csv"))))
        else:
            ...
            if max_normalixed == True:
                max_data_files = glob.glob(f"*{scale}*original_mapped_max_normalized_{scale}.csv")
        ...
        if max_normalixed == True:
            ...
            # Write all out
            fileOut = pathlib.Path(scalar_dir).joinpath(f"Data_from_all_groups_max_normalized_{scale}.csv")
            print(f"Writing out model data to {fileOut}")
            model_data.to_csv(str(fileOut), index=False)

def get_ttest_results_for_scalars(point_cloud_dir, scalar_list, canonical_geo, resels, pvalue, max_normalized=True):
    for scale in scalar_list:
        print(f"Gathering results for {scale}...")
        scalar_dir = pathlib.Path(point_cloud_dir).joinpath(scale)
        model_file = pathlib.Path(scalar_dir).joinpath(f"Data_from_all_groups_{scale}.csv")
        ...
        ttest_comparison(outname=str(scale),
                         frame=model_data,
                         canonical_geo=canonical_geo,
                         resels=resels,
                         pvalue=pvalue)
        if max_normalized==True:
            model_file = pathlib.Path(scalar_dir).joinpath(f"Data_from_all_groups_max_normalized_{scale}.csv")
            model_data = pd.read_csv(str(model_file))
            ttest_comparison(outname=f"{scale}_max_normalized",
                             frame=model_data,
                             canonical_geo=canonical_geo,
                             resels=resels,
                             pvalue=pvalue)
...
def get_data_files_for_vtk_scalars(group_list, group_identifiers, point_cloud_dir, max_normalixed=True):
    ...
    for data in data_files:
        ...
        model_data = format_for_models(data_files=data, group_list=group_list, group_identifiers=group_identifiers)
        model_data.to_csv(str(stats_dir.joinpath(f"Data_from_all_groups_{scalar_name}.csv")), sep=",", index=False)

    if max_normalixed == True:
        data_files = glob.glob(str(results_dir.joinpath(f"*_results_max_normalized.csv")))
        data_files.sort()
        ...
        for data in data_files:
            ...
            print(f"\nWriting out {scalar_name} model data...\n")
            model_data = format_for_models(data_files=data, group_list=group_list, group_identifiers=group_identifiers)
            model_data.to_csv(str(stats_dir.joinpath(f"Data_from_all_groups_{scalar_name}_max_normalized.csv")), sep=",",  index=False)
    ...
    for scalar in scalar_list:
        ...
        for p in possibilities:
            ...
            try:
                ...
                #Define the output name based on the outname and write out the dataframe
                new_name = f"{outname}{p[0]}_vs_{p[1]}"
                isolated_df.to_csv(str(stats_dir.joinpath(f"{new_name}.csv")), index=False, float_format='%.10f')
