...
def get_transform(name_in, reg, tranform_type, outDir=""):
    ...
    if outDir == "":
        ...
        transform_name = transform_dir.joinpath(f"{name_in}_{str(tranform_type)}_transformations.csv")
        correspondence_name = transform_dir.joinpath(f"{name_in}_{str(tranform_type)}_points_correspondence.csv")
...
def rigid_high_to_low(name_in, moving, fixed, iterations, tolerance, matrix=False, outDir=""):
    '''
    ...
    if outDir == "":
        ...
        matrixname = rigid_dir.joinpath(f"{name_in}_aligned_original_correspondence_matrix.csv")
...
def deformable(name_in, moving, fixed, iterations, tolerance, matrix=False):
    ...
    if matrix == False:
        ...
        np.savetxt(name_in + "_deformable_correspondence_matrix.csv", reg.P, delimiter=",", fmt='%f')
...
def map_cloud(name_in, grid, points, original, canonical_geo, scalar, scalar_values, is_3d = True):
    '''
    :param name_in:
    :param grid:
    :param values:
...
#Uses scipy interpolate to map the original values to the registered points
def map_cloud_max_normalized(name_in, grid, points, original, canonical_geo, scalar, scalar_values, is_3d=True):
    '''
...
def get_mean_case(mean_fileList, outname, canonical_geo, scalar, is_3d=True):
    ...
    mean_scalar.to_csv(outname + "mean_" + str(scalar) + "_3d.csv", index=False, sep=" ")  # pd.mean_bin
...
def initial_rigid(name_in, moving, auto3d_dir="", auto3d_ref = "trabecular", rotation_matrix="", origin=""):
    '''
...
def get_mean_case_groups(group_list, group_identifiers, bone):
    ...
    for (key, values) in dictionary.items():        
        ...
        for i in group:                
            ...
            try:
                group_list_ctth.extend(glob.glob("*" + str(i) + "*CtTh_original_mapped.csv"))                
            except:
        ...
        if len(group_list_ctth) > 1:
            ...
            get_mean_case(group_list_ctth, str(outname), mean_geo[0], "CtTh", is_3d=False)
...
def get_mean_case_groups_max_normalized(group_list, group_identifiers, bone):
    ...
    for (key, values) in dictionary.items():        
        ...
        for i in group:                
            try:
                group_list_bvtv.extend(glob.glob("*" + str(i) + "*BVTV_original_mapped_max_normalized.csv"))
            except:
                print("No members for BVTV {} group\n.".format(str(key)))
            
            try:
                group_list_da.extend(glob.glob("*" + str(i) + "*DA_original_mapped_max_normalized.csv"))                
            except:
                print("No members for DA {} group\n.".format(str(key)))
            
            try:
                group_list_ctth.extend(glob.glob("*" + str(i) + "*CtTh_original_mapped_max_normalized.csv"))                
            except:
                print(f"No members for CtTh {key} group\n.")
...
auto3d_dir = pathlib.Path(r"Z:\RyanLab\Projects\AGuerra\Nmel\Point_Clouds\Cortical\Off\Tibiotarsus_results\aligned")
...
group3_list = ["82096", "Papio_ursinus_Papio", "80774", "82097", "43086", "28256", "34712", "52206", "52223", "34714", "169430", "89365"]
...
for f in initial_rigid_fileList:
    ...
    initial_rigid(name_In, moving, auto3d_dir,
                  auto3d_ref="",
                  rotation_matrix="",
                  origin="")
    print("\n")
...
for f in mapping_fileList:
    ...
    map_cloud(name_In, grid, points, original, canonical_geo, "CtTh", scalar_values, is_3d=False)
    map_cloud_max_normalized(name_In, grid, points, original, canonical_geo, "CtTh", scalar_values, is_3d=False)
    print("\n")
map_cloud
scalar_means = pd.DataFrame(np.sort(scalar_means.values, axis=0),
...
get_mean_case(mean_fileList_CtTh, "Mean_" + str(bone) + "_CtTh", mean_geo[0], "CtTh", is_3d=False)

mean_fileList_CtTh_max_normalized = glob.glob("*CtTh_original_mapped_max_normalized.csv")
print(mean_fileList_CtTh_max_normalized)
print(len(mean_fileList_CtTh_max_normalized))

get_mean_case(mean_fileList_CtTh_max_normalized, "Mean_" + str(bone) + "_CtTh_max_normalized", mean_geo[0], "CtTh")
