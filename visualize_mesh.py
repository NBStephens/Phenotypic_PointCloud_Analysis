...
directory = pathlib.Path(
    r"Z:\RyanLab\Projects\LDoershuk\diss_pointclouds\talus\results")
os.chdir(directory)
...
mesh_name = "consolidated_talus_means.vtk"
...
input_mesh = scalar_name_cleanup(
    input_mesh=input_mesh, replace_dict=rename_dict)
input_mesh.save(f"{mesh_name}")


max_norm_list = [item for item in list(
    input_mesh.point_arrays) if "max_norm" in item]
...
max_norm_list = [item for item in list(
    stats_mesh.point_arrays) if "max_norm" in item]
stats_mesh = remove_array(input_mesh=stats_mesh, remove_arrays=max_norm_list)

stats_mesh = scalar_name_cleanup(
    input_mesh=stats_mesh, replace_dict=rename_dict)
...
merge_pdfs(pdf_directory=stats_dir, string_match="",
           out_name="radius_cortical_results.pdf", output_directory="")
...
merge_pdfs(pdf_directory=dist_plot_dir, string_match="",
           out_name="Canid_distplot_results.pdf", output_directory=dist_plot_dir)
...
bayes_outputs = ["posterior_mean", "posterior_sd",
                 "HDI_range", "posterior_ninetyseven", "posterior_three"]
...
for output in bayes_outputs:
    consolidate_vtk(input_mesh=mesh_name,
                    out_name=f"{bone}_{output}",
                    name_match=f"{output}",
                    bayes_match=True,
                    scalars=["BVTV", "DA", "Tb_N", "Tb_Sp", "Tb_Th"],
                    pairwise=False,
                    missing_scalar_name=True)  # this is if the scalar wasn't written to the array
...
input_mesh = scalar_name_cleanup(
    input_mesh=input_mesh, replace_dict=rename_dict)
input_mesh = scalar_name_suffix(
    input_mesh=input_mesh, suffix="_posterior_HDI_range")
...
rename_dict = {"_nonKWapes_": "_Homo_sapiens_",
               "_KWapes_": "_Pan_troglodytes_", "_OWM_": "_old_world_monkeys_"}
...
merge_pdfs(pdf_directory=directory, string_match="",
           out_name=f"{bone}_trabecular_results.pdf", output_directory="")
...
for output in bayes_stats:
    consolidate_vtk(input_mesh=f"canoninical_{bone}_mean.vtk",
...
stats_mesh = scalar_name_cleanup(
    input_mesh=stats_mesh, replace_dict=rename_dict)
stats_mesh = remove_array(input_mesh=stats_mesh,
                          remove_arrays=["_original_average"])
...
get_bayes_thresh_screens(input_mesh=stats_mesh,
                         scalars=["BVTV"],
                         limits=[0, 1],
...
get_bayes_thresh_screens(input_mesh=stats_mesh,
                         scalars=["BVTV", "DA", "Tb_N", "Tb_Sp", "Tb_Th"],
                         limits=[-2.0, 2.0],
                         estimate_limits=True, n_of_bar_txt_portions=11, output_type="png")

merge_pdfs(pdf_directory=directory, string_match="",
           out_name="Radius_trabecular_stats_results.pdf", output_directory="")
